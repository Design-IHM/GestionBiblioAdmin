...
export const fetchBookById = async (bookId: string): Promise<Book | null> => {
	try {
		const bookDocRef = doc(db, 'BiblioInformatique', bookId);
...
/**
 * Adds a new book to the "BiblioInformatique" collection in Firestore.
 * @param bookData - The data for the new book, excluding the ID.
 */
export const addBook = async (bookData: Omit<Book, 'id'>): Promise<void> => {
	try {
		const docRef = await addDoc(collection(db, 'BiblioInformatique'), bookData);
